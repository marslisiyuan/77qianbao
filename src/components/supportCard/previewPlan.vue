<template>
  <div>
        ...
        <div class="previewPlan">
            ...
            <p>周转金：￥{{Math.ceil(parseFloat(Order['40'])+parseFloat(Order['41']))}}</p>
            <p>还款笔数费：￥{{parseFloat(Order['9']).toFixed(2)}}</p>
            <div class="content">
                <router-link tag="button" to="/supportCard/planRecord">预览详细计划</router-link>
                <button @click="backed">修改计划</button>
            </div>
        </div>
        <div class="footerbutton">
            <button @click="submitPlan($event)">提交计划</button>
        </div> 
         <!-- 弹出 -->
...
<script>
    export default {
        ...
        console.log(sessionStorage.getItem('smallPlan'))
        },
        methods:{
          backed(){
              window.history.go(-1)
          ...
          submitPlan(event){
                var jsona=$.parseJSON(JSON.parse(sessionStorage.getItem('plan'))['57'])
                var jsonArray=[]
                for(var i=0;i<jsona.length;i++){
                     var obj={}
                     for(var tmp in jsona[i]){
                         if(tmp=='status'){
                          obj.status=jsona[i][tmp]
                         }else if(tmp=='type'){
                          obj.type=jsona[i][tmp]
                         }else if(tmp=='bindID'){
                            obj.toBindId=jsona[i][tmp] 
                            obj.fromBindId=jsona[i][tmp] 
                         }else if(tmp=='money'){
                            obj.toMoney=jsona[i][tmp] 
                            obj.fromMoney=jsona[i][tmp] 
                         }else if(tmp=='time'){
                            obj.planTime=new Date(jsona[i][tmp].replace(/-/g,'/')).getTime()
                         }else if(tmp=='groupNum'){
                             obj.groupNum=jsona[i][tmp]
                         }
                     }
                     jsonArray.push(obj)
                 }
                ...
                if(sessionStorage.getItem('smallPlan')){
                  var jsonArray=sessionStorage.getItem('smallPlan')
                }else{
             ...
             var reserveMoney=Math.ceil(parseFloat(this.Order['40'])+parseFloat(this.Order['41'])),
                 ...
                 MAC=$.md5('0700490016'+this.Order['8']+this.Order['40']+startDate+endDate+this.Order['7']+this.Order['9']+JSON.parse(sessionStorage.getItem('next')).state+merchantNo+sessionStorage.getItem('acqcode')+jsonArray+this.version+'21E4ACD4CD5D4619B063F40C5A454F7D')
                ...
                var _vm=this
                 this.$http.post(this.url,{
                    ...
                    '12':this.Order['7'],
                    '13':this.Order['9'],
                    '20':JSON.parse(sessionStorage.getItem('next')).state,
                    '42':merchantNo,
